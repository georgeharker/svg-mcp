...
class PreviewServer:
    """A singleton HTTP+SSE server publishing snapshots of the active document.
    ...
    def __init__(self) -> None:
        ...
        self._index_json = '{"active": null, "documents": []}'
        self._clients: set[queue.Queue[int]] = set()
    ...
    def ensure_running(self, host: str = "127.0.0.1", port: int | None = None) -> tuple[str, int]:
        """Start the server if needed and return ``(url, port)``. Idempotent.

        Tries the requested port (default 8808); if it is taken, binds an ephemeral port and
        reports the one actually bound. A server already running is left as-is.
        """
        with self._lock:
            if self._httpd is not None and self._url is not None:
                return self._url, self._httpd.server_address[1]
            ...
            self._httpd = httpd
            self._url = f"http://{host}:{bound}/"
            thread = threading.Thread(
                target=httpd.serve_forever, name="svg-mcp-preview", daemon=True
            )
            thread.start()
            return self._url, bound

    # -- snapshot publishing ------------------------------------------------

    def publish(self, *, active_id: str | None, sources: dict[str, str], index_json: str) -> None:
        """Replace the served snapshot and notify connected browsers to refresh.

        ``sources`` maps document_id -> serialized SVG; it is merged in (so previously-captured
        documents stay renderable) and pruned to the ids present in ``index_json``.
        """
        gen: int
        with self._lock:
            ...
            self._sources = {k: v for k, v in self._sources.items() if k in live}
            self._gen += 1
            gen = self._gen
    ...
    def _resolve(self, doc_id: str) -> str | None:
        with self._lock:
            if doc_id == "active":
                doc_id = self._active or ""
            return self._sources.get(doc_id)

    def state(self) -> tuple[int, str | None, str]:
        with self._lock:
            return self._gen, self._active, self._index_json
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802 (stdlib naming)
            ...
            if parts == ["events"]:
                return self._sse()
            if parts == ["version"]:
                gen, active, _ = self.preview.state()
                return self._text(
                    json.dumps({"gen": gen, "active": active, "vector": rsvg_available()}),
                    "application/json",
                )
            if parts == ["documents"]:
                _, _, index_json = self.preview.state()
                return self._text(index_json, "application/json")

            # /{doc}/svg | /{doc}/render | /{doc}/export.{fmt}
            if len(parts) == 2:
                doc_id, leaf = parts
                source = self.preview._resolve(doc_id)
                if source is None:
        ...
        def _sse(self) -> None:
            ...
            self.end_headers()
            q = self.preview.subscribe()
            try:
                self._sse_send(self.preview.state()[0])  # prime with current generation
                while True:
                    try:
...
function pannable(){
  // The scaled stage overflows main once it's larger than the viewport.
  return view.scrollWidth > view.clientWidth + 1 || view.scrollHeight > view.clientHeight + 1;
}
function applyScale(){
...
function buildSaveMenu(){
  ...
  document.getElementById('save-menu').innerHTML = formats.map(([f,label]) =>
    '<a href="/active/export.' + f + '" download="drawing.' + f + '">' + label + '</a>'
  ).join('');
}
...
async function refresh(){
  ...
  try {
    const idx = await (await fetch('/documents', {cache:'no-store'})).json();
  ...
  if (mode === 'png') {
    ...
    img.src = '/active/render?v=' + gen;
  } else {
    const svg = await (await fetch('/active/svg?v=' + gen, {cache:'no-store'})).text();
