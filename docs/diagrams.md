...
`line` (points/area options), `donut`, `scatter`, `histogram`, `sparkline`, `radar`. Ticks land on the 1/2/5
...
| `axes` (the list) / `rings` / `r_max` / `ring_labels` | radar | The wheel. `axes` names the spokes — **at least three**, drawn clockwise from 12 o'clock — and every series carries one value per spoke, all read against **one shared radial scale** (so only commensurable spokes belong on one radar). `rings` is a target count the 1/2/5 ladder rounds off (~4 by default), drawn as **n-gons, not circles**, so a gridline is parallel to the polygon it measures everywhere; the rim is always a ring. `r_max` pins the rim so two radars compare — a reading past a pinned rim is drawn **past the rim**, not clipped, because a radar's rim is a gridline and a clipped polygon would read as a datum sitting exactly at the maximum. `ring_labels` writes the ruler up the 12 o'clock spoke, formatted by `value_format`. **Negative values are refused**: a radius runs one way out of the centre, and a radar has no zero line to cross — that data wants a bar chart. The cartesian `axes=` argument is ignored, like a donut's. |
| `fill` / `marker` / `open` | radar | `fill` washes each polygon at 0.15 fill-opacity **behind every outline** (all washes, then all outlines, then all marks — a wash over another profile would tint it into a colour neither series wears). `marker` is the full glyph vocabulary at each vertex (`none` by default: the corners already mark the readings), `open` draws them hollow. |
